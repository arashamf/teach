...
fn main() {
    ...
    use std::io::stdin;
    let apartments:u32 = stdin().lines().next().unwrap().unwrap().parse().expect("wrong input");
    let number_aparts:u32 = stdin().lines().next().unwrap().unwrap().parse().expect("wrong input");

    let floor: u32;
    if (number_aparts%apartments) > 0   {
        floor = (number_aparts/ apartments) + 1; }
    else {
        floor = number_aparts/ apartments;
    }
    println!("Квартира с номером {} находится на {} этаже", number_aparts, floor);
}